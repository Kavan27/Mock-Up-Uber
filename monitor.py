...
class Monitor:
    ...
    def _average_total_distance(self) -> float:
        ...
        for activities in self._activities[DRIVER].values():
            for x in range(len(activities) - 1):
                avg_distance += manhattan_distance(activities[x].location,
    ...
    def _average_ride_distance(self) -> float:
        ...
        for activities in self._activities[DRIVER].values():
            for x in range(len(activities) - 1):
                if (activities[x].description == PICKUP) and \
                        (activities[x + 1].description == DROPOFF):
                    avg_distance += \
                        manhattan_distance(activities[x].location,
                                           activities[x + 1].location)
            count += 1
        return avg_distance / count
