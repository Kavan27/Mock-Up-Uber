...
class DriverRequest(Event):
    ...
    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        ...
        events = []
        if self.driver not in dispatcher.r_drivers:
            dispatcher.r_drivers.append(self.driver)
...
class Cancellation(Event):
    ...
    === Attributes ===
    timestamp: A timestamp for cancellation.
    rider: A rider.
    """
...
class Pickup(Event):
    ...
    def __str__(self) -> str:
        ...
        """
        return "Driver {} picks up {} at time {}: Request a rider".\
            format(self.driver, self.rider, self.timestamp)
    ...
    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        ...
        if self.rider.status == WAITING:
            ...
            events.append(Dropoff(self.timestamp + time,
                                  self.rider, self.driver))
            self.rider.status = SATISFIED
            dispatcher.remove_rider(self.rider)
...
class Dropoff(Event):
    ...
    def __str__(self) -> str:
        ...
        """
        return "Driver {} drops off {} at time {}: Request a rider".\
            format(self.driver, self.rider, self.timestamp)
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if event_type == "DriverRequest":
                ...
                speed = int(tokens[4])
                event = DriverRequest(timestamp, Driver(name, loc, speed))
                events.append(event)
                ...
                rider = Rider(name, patience, origin, destination)
